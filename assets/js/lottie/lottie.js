...
const onZoloSplineInteraction = (doc = document, win = window) => {
    ...
    scriptElement.onload = () => {
        ...
        let splineElements = doc.querySelectorAll('.wp-block-zolo-lottie-image');
        ...
        if (splineElements.length > 0) {
            ...
            splineElements.forEach((element) => {
                ...
                // Listen for the 'load' event from the lottie-image element
                let splineViewer = element.querySelector('lottie-image');
                if (splineViewer) {
                    //hide #logo that is inside shadowRoot of lottie-image
                    setTimeout(() => {
...
let zoloSplineLoaderElements = document.querySelectorAll('.wp-block-zolo-lottie-image .zolo-spline-loader');
