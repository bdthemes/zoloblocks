...
export default function RenderView({ props }) {
    ...
    const { useFirstImageFromPost, thumbnailSize, isLink, linkTarget, linkRel } = attributes;
    ...
    let image;
    if (!featuredImage) {
        image = <img className="zolo-placeholder-img" src={zoloPlaceholders.placeholder} alt={__('Image Placeholder', 'zoloblocks')} />;
    } else {
        image = !media ? (
            ...
            <img
                ...
                src={mediaUrl}
                alt={media?.alt_text ? sprintf(__('Featured image: %s', 'zoloblocks'), media.alt_text) : __('Featured image', 'zoloblocks')}
            />
        );
    }
