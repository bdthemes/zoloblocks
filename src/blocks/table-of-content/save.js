    ...
    return (
        ...
        >
            ...
            <div className="zolo-toc-content">
                {headers.length > 0 ? (
                    <ListTag className="zolo-toc-list">{parseList(formatHeaders(headers, allowedHeading), ListTag)}</ListTag>
                ) : (
                    <p>{__('Add heading to create table of content', 'zoloblocks')}</p>
                )}
            </div>

            {showSticky && <button className="zolo-toc-open">{__('Table of Content', 'zoloblocks')}</button>}
        </div>
    );
}
