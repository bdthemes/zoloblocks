...
const AuthorMeta = ({ post, meta }) => {
    ...
    const author = useSelect((select) => (authorId ? select('core').getUser(authorId) : null), [authorId]);
    const authorName = author?.name || __('Author Name', 'zoloblocks');
    const authorLink = author?.link || '#';

    const content = (
        <>
            {meta?.showIcon !== 'none' && <span className="zolo-icon" dangerouslySetInnerHTML={{ __html: meta?.icon }} />}
...
const DateMeta = ({ post, meta }) => {
    const postDate =
        meta?.dateType === 'post_modified' ? (post?.modified ? new Date(post.modified) : null) : post?.date ? new Date(post.date) : null;
...
const ReadingTimeMeta = ({ post, meta }) => {
    const rawContent = useSelect((select) => (post?.content ? select('core/editor').getEditedPostContent() : ''), [post?.content]);
...
const MetaItem = ({ meta, post }) => {
    ...
    return <div className={`zolo-meta-info ${meta.type}`}>{getMetaComponent(meta.type)}</div>;
};
