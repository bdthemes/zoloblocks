                    ...
                    <div className="dnd-container" key={item.id}>
                        ...
                        <SortableItem key={item.id} id={item.id}>
                            <ZoloCorePanelBody title={`Item ${index + 1} - ${layerLabel(item.layer)}`} initialOpen={false}>
                                ...
                                <ZoloToggleControl
                                    label={__('Enable Tooltip', 'zoloblocks')}
                                    checked={item.enableTooltip || false}
                                    onChange={(value) => updateCircleItem(index, 'enableTooltip', value)}
                                />

                                {item.enableTooltip && (
                                    <ZoloTextControl
                                        label={__('Tooltip Text', 'zoloblocks')}
                                        value={item.tooltipText || ''}
                                        onChange={(value) => updateCircleItem(index, 'tooltipText', value)}
                                        placeholder={__('Enter tooltip text...', 'zoloblocks')}
                                    />
                                )}
                            </ZoloCorePanelBody>
                        </SortableItem>
