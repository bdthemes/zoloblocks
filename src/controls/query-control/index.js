import { ZoloBaseControl, ZoloSelectControl, ZoloInputControl } from '../core-controls';
...
const QueryControl = ({ attributes, setAttributes }) => {
    ...
    const afterSourceControl = applyFilters(
        'zolo.blocks.queryControl.afterSource',
        [],
        {
            attributes,
            setAttributes,
            ...
            {'current_post' !== postQuery?.postType && (
                <>
                    ...
                    <TabDynamicControl
                        ...
                        include={
                            <>
                                <ZoloBaseControl label={__('Include By', 'zoloblocks')} className="zolo-flex-col-control">
                                    <Select2
                                ...
                                {postQuery?.postIncludeBy?.some((item) => item.value === 'authors') && (
                                    <ZoloBaseControl label={__('Authors', 'zoloblocks')}>
                                        <Select2
                                            classNamePrefix="zolo-select"
                                            options={AUTHOR_LISTS}
                                            value={postQuery?.postIncludeAuthors}
                                            onChange={(postIncludeAuthors) =>
                                                setAttributes({ postQuery: { ...postQuery, postIncludeAuthors } })
                                            }
                                            isMulti={true}
                                            closeMenuOnSelect={false}
                                        />
                                ...
                                {postQuery?.postIncludeBy?.some((item) => item.value === 'terms') &&
                                    zoloAllTaxonomies.map((tax, index) => (
                                        <ZoloBaseControl label={tax.label} key={index}>
                                            <Select2
                                                classNamePrefix="zolo-select"
                        ...
                        exclude={
                            <>
                                <ZoloBaseControl label={__('Exclude By', 'zoloblocks')} className="zolo-flex-col-control">
                                    <Select2
                                        classNamePrefix="zolo-select"
                                        options={EXCLUDE_BY}
                                ...
                                {postQuery?.postExcludeBy?.some((item) => item.value === 'authors') && (
                                    <ZoloBaseControl label={__('Authors', 'zoloblocks')}>
                                        <Select2
                                            ...
                                            onChange={(postExcludeAuthors) =>
                                                setAttributes({ postQuery: { ...postQuery, postExcludeAuthors } })
                                            }
                                            isMulti={true}
                                            closeMenuOnSelect={false}
                                        />
